    ...
    request(type: string, url: string, content: any = {}, headers: any = {'Accept': 'application/json', 'Content-Type': 'application/json'}): any{
        switch(type){
            case 'GET':{
                ...
                //     this.updateToken();
                // }
                var req = this.http.get(url, {headers: headers});
                return req;
            }
            case 'POST':{
                // if(this.isAuthenticated()){
                //     this.updateToken();
                // }
                var req = this.http.post(url, content, {headers: headers});
                return req;
            }
            case 'PUT':{
                // if(this.isAuthenticated()){
                //     this.updateToken();
                // }
                var req = this.http.put(url, content, {headers: headers});
                return req;
            }
            case 'DELETE':{
                // if(this.isAuthenticated()){
                //     this.updateToken();
                // }
                var req = this.http.delete(url, {headers: headers});
                return req;
            }
        }
    }
